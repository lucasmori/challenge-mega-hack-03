...
export class UserManager {
  public async enrollAdmin(req: Request, res: Response): Promise<void> {
    try {
      ...
      const caInfo = ccp.certificateAuthorities["ca.vtex.com"];
      ...
      if (adminExists) {
        ...
        res
          .status(409)
          .send("An identity for the admin already exists in the wallet");
        return;
      }
      ...
      res
        .status(200)
        .send("Successfully enrolled admin and imported it into the wallet");
    } catch (error) {
  ...
  public async registerUser(req: Request, res: Response): Promise<void> {
    try {
      ...
      const cpf = req.body.user;

      // Check to see if we've already enrolled the user.
      const userExists = await wallet.exists(`${cpf}`);
      if (userExists) {
        res
          .status(409)
          .send(`An identity for the user ${cpf} already exists in the wallet`);
        return;
      }
      ...
      if (!adminExists) {
        res
          .status(404)
          .send(`An identity for the ADMIN does not exist in the wallet`);
        return;
      }
      ...
      await wallet.import(`${cpf}`, userIdentity);

      console.log(`Registered user`);
      res.status(200).send("true");
    } catch (error) {
      res.status(500).send(`User with the cpf ${error} already exists`);
    }
  }
